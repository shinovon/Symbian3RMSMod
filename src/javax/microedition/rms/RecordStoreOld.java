...
public class RecordStoreOld extends RecordStore
{
    /**
     * Authorization modes.
    ...
    throws RecordStoreNotOpenException
    {
        return new com.nokia.mj.impl.rms.RecordEnumerationImpl(iRms, iSync, aFilter, aComparator, aKeepUpdated, aTags);
    }
    ...
    RecordStoreOld(RecordStoreOpenArgs aArgs) throws RecordStoreNotFoundException, RecordStoreException
    {
    	super();
    ...
    throws RecordStoreException, RecordStoreFullException, RecordStoreNotFoundException
    {
        ...
        synchronized (iOpenRecordStores)
        {
            RecordStoreOld recordStore = findRecordStore(aArgs.getRecordStoreName());
            if (recordStore == null)
            {
                recordStore = new RecordStoreOld(aArgs);
    ...
    private static RecordStoreOld findRecordStore(RmsName aName)
    {
        int num = iOpenRecordStores.size();
        for (int i = 0; i < num; i++)
        {
            RecordStoreOld rs = (RecordStoreOld) iOpenRecordStores.elementAt(i);
